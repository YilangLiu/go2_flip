...
class UnitreeGo2EnvConfig(BaseEnvConfig):
    kp: Union[float, jax.Array] = 30.0 #30.0
    kd: Union[float, jax.Array] = 0.0 #0.0
    # kp: Union[float, jax.Array] = jnp.array([30.0, 30.0, 30.0, 
    #                                          30.0, 30.0, 30.0,
    #                                          30.0, 30.0, 35.0,
    #                                          30.0, 30.0, 35.0,])
...
class UnitreeGo2FlipBipedalEnvConfig(UnitreeGo2EnvConfig):
    pass

class UnitreeGo2FlipBipedal(UnitreeGo2Env):
    def __init__(self, config: UnitreeGo2FlipBipedalEnvConfig = UnitreeGo2FlipBipedalEnvConfig()):
        super().__init__(config)
        self.joint_range = jnp.array(
            [
                [-0.5, 0.5],
        ...
        self._init_q = jnp.array(self.sys.mj_model.keyframe("bipedal").qpos)
        self._default_pose = self.sys.mj_model.keyframe("bipedal").qpos[7:]
        self._end_pose = self.sys.mj_model.keyframe('bipedal_head').qpos
        self._init_quat = self._init_q[3:7]
        self._global_gravity = jnp.array([0, 0, -1])
    ...
    def step(
        ...
        # reward
        # orientation control 
        current_time = state.info["step"] * self.dt 
        phase = jnp.clip(current_time - 0.5, 0, 0.5)
        quat_pitch = math.quat_rot_axis(jnp.array([0, 1, 0]), 2 * phase * jnp.pi + jnp.pi/2)
        desired_base_quat = math.quat_mul(quat_pitch, self._init_quat)
        inv_desired_base_quat = math.quat_inv(desired_base_quat)
        desired_projected_gravity = math.rotate(self._global_gravity, inv_desired_base_quat)
        
        inv_base_quat = math.quat_inv(pipeline_state.qpos[3:7])
        projected_gravity = math.rotate(self._global_gravity, inv_base_quat)
        orientation_diff = -jnp.sum(jnp.square(projected_gravity - desired_projected_gravity))

        # ang_vel_y
        ang_vel_y = jnp.clip(xd.ang[self._torso_idx - 1][1], -7.2, 7.2)
        reward_ang_vel_y = -ang_vel_y * jnp.logical_and(current_time > 0.5, current_time<1.0) # TODO: double check 


        # ang_vel_z
        reward_ang_vel_z = -jnp.abs(xd.vel[self._torso_idx - 1][2])

        # lin_vel_z
        lin_vel_z = jnp.clip(pipeline_state.qvel[2], max=3)
        reward_lin_vel_z = lin_vel_z * jnp.logical_and(current_time > 0.5, current_time < 0.75) # TODO: current time maybe smaller than 0.75

        # feet_height_before_flip 
        foot_pos = pipeline_state.site_xpos[
            self._feet_site_id[2:]
        ]  # pytype: disable=attribute-error
        foot_contact_z = jnp.clip(foot_pos[:, 2] - self._foot_radius, min=0, max=1)
        reward_feet_height_before_flip = -jnp.sum(foot_contact_z) * (current_time < 0.5)


        # feet_height_after_flip 
        foot_pos = pipeline_state.site_xpos[
            self._feet_site_id[:2]
        ]  # pytype: disable=attribute-error
        foot_contact_z = jnp.clip(foot_pos[:, 2] - self._foot_radius, min=0, max=1)
        reward_feet_height_after_flip = -jnp.sum(foot_contact_z) * (current_time > 1.0)

        # height control 
        target_height = self._default_pose[2] * (current_time < 0.5) + self._end_pose[2] * (current_time > 1.0)
        reward_height_control = -jnp.square(target_height - pipeline_state.qpos[2]) * jnp.logical_or(current_time < 0.5, current_time > 1.0)

        # TODO: action_symmetry (optional)
        
        
        # gravity_y 
        reward_gravity_y = -jnp.square(projected_gravity[1]) * jnp.logical_or(current_time < 0.5, current_time > 1.0)

        # TODO: feet distance (optional)

        # action rate 
        reward_action_rate = -jnp.sum((ctrl - state.info["last_ctrl"]) ** 2)


        # (simple terminal goal)

        weights = jnp.array([2000, 2000, 5000,        # base position
                             200000, 0, 0, 0,    # base orientation in quaternion
                             800, 800, 800,        # FL joint angle
                             800, 800, 800,        # FR joint angle
                             800, 800, 800,        # RL joint angle
                             800, 800, 800,        # RL joint angle
                             
                             0.0001, 0.0001, 0.0001,        # base linear velocity 
                             0.0001, 0.0001, 0.0001,        # base angular velocity in yaw pitch roll
                             0.1, 0.1, 0.1,        # FL joint velocity 
                             0.1, 0.1, 0.1,        # FR joint velocity 
                             0.1, 0.1, 0.1,        # RL joint velocity 
                             0.1, 0.1, 0.1])       # RR joint velocity
        ...
        # reward for multi stage
        # reward = (
        #     orientation_diff * 1.0
        #     # + reward_ang_vel_y * 5.0
        #     + reward_ang_vel_z * 1.0
        #     + reward_lin_vel_z * 0.1
        #     + reward_feet_height_before_flip * 15.0
        #     + reward_feet_height_after_flip * 15.0
        #     + reward_height_control * 10.0
        #     # + reward_gravity_y * 10.0
    ...
    def reset(self, rng: jax.Array) -> State:
        ...
        state.info["ang_vel_tar"] = jnp.array([0.0, 0.0, 0.0])
        state.info["yaw_tar"] = 0.0
        state.info["last_ctrl"] = jnp.zeros(12)
        return state

class UnitreeGo2BipedalWalk(UnitreeGo2Env):
    def __init__(
        self, config: UnitreeGo2EnvConfig = UnitreeGo2EnvConfig()
    ):
        super().__init__(config)
        self.joint_range = jnp.array(
            [
        ...
        self._init_q = jnp.array(self.sys.mj_model.keyframe("bipedal").qpos)
        self._default_pose = self.sys.mj_model.keyframe("bipedal").qpos[7:]
        self._rear_feet_id = self._feet_site_id[2:]
    ...
    def reset(self, rng: jax.Array) -> State:  # pytype: disable=signature-mismatch
        ...
        state_info = {
            "rng": rng,
            "pos_tar": jnp.array([0.282, 0.0, 0.46]),
            "vel_tar": jnp.array([0.0, 0.0, 0.0]),
            "ang_vel_tar": jnp.array([0.0, 0.0, 0.0]),
            "yaw_tar": 0.0,
            "step": 0,
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        # reward
        # gaits reward
        z_feet = pipeline_state.site_xpos[self._rear_feet_id][:, 2]
        duty_ratio, cadence, amplitude = self._gait_params[self._gait]
        phases = self._gait_phase[self._gait]
        z_feet_tar = get_foot_step(
            duty_ratio, cadence, amplitude, phases, state.info["step"] * self.dt
        ...
        reward_pos = -jnp.sum((head_pos - pos_tar) ** 2)
        # stay upright reward
        vec_tar = jnp.array([0.0, 1.0, 0.0])
        vec = math.rotate(vec_tar, x.rot[0])
        # reward_upright = -jnp.sum(jnp.square(vec - vec_tar))


        reward_upright = -jnp.sum(jnp.square(jnp.array([0.707, 0, -0.707, 0]) - x.rot[0]))
        ...
        # reward_vel = -jnp.sum((vb[:2] - state.info["vel_tar"][:2]) ** 2)
        # reward_ang_vel = -jnp.sum((ab[2] - state.info["ang_vel_tar"][2]) ** 2)
        reward_vel = -jnp.sum((xd.vel[self._torso_idx - 1][:2] - state.info["vel_tar"][:2]) ** 2)
        reward_ang_vel = -jnp.sum((xd.ang[self._torso_idx - 1][2] - state.info["ang_vel_tar"][2]) ** 2)
...
brax_envs.register_environment("unitree_go2_bipedal_flip", UnitreeGo2FlipBipedal)
brax_envs.register_environment("unitree_go2_bipedal_walk", UnitreeGo2BipedalWalk)